...
- [`Release build`](../.github/workflows/release.yml) — extension zip + `.pkg` / `.ipa`; checkboxes `submit_mac_app_store` / `submit_ios_app_store` (manual default off). Tag `v*` always attempts both submits. Builds always run; missing ASC secrets fail inside the Mac/iOS jobs (same pattern as redd-block).
